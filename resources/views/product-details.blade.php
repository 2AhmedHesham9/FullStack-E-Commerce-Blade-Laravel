    ...
    <section class="product-single container">
        <div class="row">
                    ...
                    <div class="product-single__image">
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                ...
                                <div class="swiper-slide product-single__image-item">
                                    <img loading="lazy" class="h-auto"
                                        src="{{ asset('uploads/products') }}/{{ trim($image) }}" width="674"
                                        height="674" alt="{{ $product->name }}" />
                    ...
                    <div class="product-single__thumbnail">
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                ...
                                <div class="swiper-slide product-single__image-item"><img loading="lazy" class="h-auto"
                                        src="{{ asset('uploads/products/thumbnails') }}/{{ trim($image) }}" width="104"
                                        height="104" alt="" />
                                </div>
                                @endforeach
            ...
            <div class="col-lg-5">
                ...
                @elseif(Auth::check() && $sizes->first() =='' ? 0 :1)
                <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                    @csrf
                    <div class="product-single__addtocart">
                        <div class="qty-control position-relative">
                            <input type="number" name="quantity" value="1" min="1"
                                max="{{  $sizes->first() =='' ? 0: $sizes->first()->pivot->quantity  }}"
                                class="qty-control__number text-center" id="quantity-input">
                        ...
                        <div class="select">
                            <select name="size" id="option-select" required class=" select">
                                {{-- <option value="">Select Size</option> --}}
                                @foreach ($sizes as $size )
                                <option value="{{ $size->pivot->quantity . ',' . $size->name   }}  ">
                                    {{ $size->name }}</option>

                                {{-- <input type="hidden" name="proqty" value="{{  $size->pivot->quantity }}"> --}}
                ...
                @else
                <button class="btn btn-danger btn-addtocart">Sold Out
                </button>
                @endif
                ...
                <div class="product-single__addtolinks">
                    ...
                    <p style="display: none">
                        {{ $rowId =Cart::instance('wishlist')->content()->where('id', '=', $product->id)->first()->rowId
                        }}
                    </p>
                    <form action="{{ route('wishlist.item.remove',['rowId'=> $rowId ]) }}" method="post" id="wishform">
                        @csrf
                        @method('DELETE')
                        <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist filled-heart"
                            onclick="document.getElementById('wishform').submit(); "><svg width="16" height="16"
                    ...
                    <form action="{{ route('wishlist.add') }}" method="post" id="wishlistform">
                        ...
                        <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist"
                            onclick="document.getElementById('wishlistform').submit();"><svg width="16" height="16"
                                viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <use href="#icon_heart" />
                ...
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide product-card">
                        <div class="pc__img-wrapper">
                            <a href="{{ route('product.details',['slug_product' => $rproduct->slug]) }}">
                                <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $rproduct->image }}"
                                    width="330" height="400" alt="{{ $rproduct->name }}" class="pc__img">
                                @foreach (explode(',',$rproduct->images) as $rimage)
                                <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $rimage }}" width="330"
...
<script>
    ...
    $('#option-select').on('change', function() {
        var selectedValue = $(this).val().split(',')[0]; // Get selected option value


        if( parseInt($('#quantity-input').val()) > selectedValue)
        {
            alert("Avilable Quantity is " + selectedValue)
        }

        var quantity = selectedValue  ; // Get quantity for selected size

        // Update the displayed quantity
        $('#quantity-input').attr('max', quantity); // Change input max value
        $('#quantity-input').val(1).attr('max', quantity); // Reset value to 1
    });
});
