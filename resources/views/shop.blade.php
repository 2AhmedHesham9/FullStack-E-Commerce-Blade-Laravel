    ...
    <section class="shop-main container d-flex pt-4 pt-xl-5">
                                ...
                                <li class="list-item">
                                    <span class="menu-link py-1">
                                        <input type="checkbox" name="categories" value="{{ $category->id }}"
                                            class="chk-category" id="{{$category->name}}" @if(in_array($category->id,
                                        explode(',',
                    ...
                    <div id="accordion-filter-size" class="border-0 accordion-collapse collapse show"
                        ...
                        <div class="px-0 pb-0 accordion-body">
                            <div class="flex-wrap d-flex">
                                <a href="#" data-size=""
                                    class="mb-3 size-link swatch-size btn btn-sm btn-outline-light me-3 js-filter    {{ $f_product_size=='' ? 'swatch_active':'' }}    "
                                    value="">All</a>
                                @foreach ($productSizes as $productsize )

                                <a href="#" data-size="{{ $productsize->name }}"
                                    class="mb-3 size-link swatch-size btn btn-sm btn-outline-light me-3 js-filter {{ $f_product_size==$productsize->name  ? 'swatch_active':'' }}"
                                ...
                                <li class="list-item">
                                    <span class="menu-link py-1">
                                        <input type="checkbox" name="brands" value="{{ $brand->id }}" class="chk-brand"
                                            id="{{ $brand->name }}" @if(in_array($brand->id, explode(',', $f_brands)))
                ...
                <div class="accordion-item mb-4">
                    ...
                    <div id="accordion-filter-price" class="accordion-collapse collapse show border-0"
                        ...
                        <input class="price-range-slider" type="text" name="price_range" value=""
                            data-slider-min="{{ $min_product_price }}" data-slider-max="{{ $max_product_price}}"
        ...
        <div class="shop-list flex-grow-1">
                        ...
                        <div class="slide-split h-100 d-block d-md-flex overflow-hidden">
                            <div class="slide-split_text position-relative d-flex align-items-center"
                                ...
                                <div class="slideshow-text container p-3 p-xl-5">
                                    <h2
                                        class="text-uppercase section-title fw-normal mb-3 animate animate_fade animate_btt animate_delay-2">
                                        {{ $product->brand->name }} <br /><strong>{{ $product->category->name
                                            }}</strong></h2>
                                    <p class="mb-0 animate animate_fade animate_btt animate_delay-5">ATSH Store is the
                                        best place to
                                        update your look. Add a title edge with new styles and new colors, or go for
                                        timeless pieces.</h6>
                                </div>
                            ...
                            <div class="slide-split_media position-relative">
                                <div class="slideshow-bg" style="background-color: #f5e6e0;">
                                    <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $product->image }}"
                                        ...
                                        class="slideshow-bg__img object-fit-cover co" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="d-flex justify-content-between mb-4 pb-md-2">
                ...
                <div
                    class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1 text-center">

                    <select
                        class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0 text-center  ml-1 "
                        aria-label="Page Size" name="pagesize" id="pagesize">
                        <option value="12" {{ $size==12?'selected':'' }}>Show</option>
                        <option value="24" {{ $size==24?'selected':'' }}>24 Per Page </option>
                        <option value="48" {{ $size==48?'selected':'' }}>48 Per Page </option>
                        <option value="102" {{ $size==102?'selected':'' }}>102 Per Page </option>
                    </select>

                    <select
                        class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0 text-center mr-1 "
                        aria-label="Sort Items" name="orderby" id="orderby">
                        <option value="def" {{ $order==12?'selected':'' }}>Default</option>
                    ...
                    <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                        <div class="pc__img-wrapper">
                            <div class="swiper-container background-img js-swiper-slider"
                                ...
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide">
                                        <a href="{{ route('product.details',['slug_product' => $product->slug]) }}"><img
                                                loading="lazy"
                                                src="{{ asset('uploads/products') }}/{{ trim($product->image)  }}"
                                                width="330" height="400" alt="{{ $product->name }}" class="pc__img"></a>
                                    </div>
                                    @foreach (explode(',',$product->images) as $image)
                                    <div class="swiper-slide">
                                        <a href="{{ route('product.details',['slug_product' => $product->slug]) }}"><img
                                                loading="lazy" src="{{ asset('uploads/products') }}/{{ trim($image)  }}"
                                                width="330" height="400" alt="{{ $product->name }}" class="pc__img"></a>
                                    </div>
                                    @endforeach
                                </div>
                            ...
                            <a href="{{ route('cart.index') }}"
                                class="pc__atc btn btn-warning  anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium">Go
                                To Cart</a>
                            @else
                            <form action="{{   route('product.details',['slug_product' => $product->slug])  }}"
                                method="get">
                                <button type="submit"
                                    class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium js-add-cart  "
                                    data-aside="cartDrawer" title="Show Details">Show Details</button>
                            </form>
                        ...
                        <div class="pc__info position-relative">
                            ...
                            <h6 class="pc__title"><a
                                    href="{{ route('product.details',['slug_product' => $product->slug]) }}">{{
                                    $product->name }}</a></h6>
                            ...
                            <p style="display: none">
                                {{ $rowId =Cart::instance('wishlist')->content()->where('id', '=',
                                $product->id)->first()->rowId }}
                            </p>
                            <form action="{{ route('wishlist.item.remove',['rowId'=>$rowId]) }}" method="post">
                                @csrf
                                @method('delete')
                                <button type="submit"
                                    class="position-absolute top-0 end-0 bg-transparent border-0 js-add-wishlist filled-heart "
            ...
            <div class="flex items-center justify-between flex-wrap gap10 wgp-pagination">
                ...
                {{-- withQueryString() take all url variables
                to ensure that
                current query string parameters are included
                in the pagination link --}}
            </div>
...
<form action="{{ route('shop.index') }}" id="frmfilter" method="GET">
    ...
    <input type="hidden" name="product_sizes" id="hdnproduct_sizes" value="{{ $f_product_size }}">
